...
int main(){
    ...
    rep(i,0,n-8){
        rep(j,0,m-8){
            ...
            rep(k,0,3){
                rep(l,0,3){
                    if(s[i+k][j+l]!='#') kuro = false;
                    if(s[i+6+k][j+6+l]!='#') kuro = false;
                    break;
                }
            }
            if(!kuro) continue;
            bool shiro = true;
            rep(k,0,4){
                if(s[i+k][j+3]!='.') shiro = false;
                if(s[i+5+k][j+5]!='.') shiro = false;
            }
            rep(k,0,4){
                if(s[i+3][j+k]!='.') shiro = false;
                if(s[i+5][j+5+k]!='.') shiro = false;
            }
            if(!shiro) continue;
            if(kuro and shiro){
                vec.emplace_back(i+1, j+1);
            }
