...
int main() {
    ...
    auto ok = [&](P now, P nv){
        return 
        (0<=nv.first and nv.first <h) and
        (0<=nv.second and nv.second <w) and
        (m[now.first][now.second] < m[nv.first][nv.second]);
    };

    map<P, ll> dp;
    // dp[v]:= vから探索可能な経路の個数
    auto dfs = [&](auto dfs, P now) -> ll {
        if(dp[now]) return dp[now];
        ll ret = 0;
        rep(i,0,4){
            P nv = {now.first+dy[i], now.second+dx[i]};
            if(!ok(now, nv)) continue;
            ret += (dfs(dfs, nv) + 1)%mod; 
        }
