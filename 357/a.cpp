...
int main() {
    ...
    vl a(n); cin >> a;          
    Graph G(n);
    rep(i,0,n) G[i].emplace_back(--a[i]);
    ...
    for(int i = 0; i < n; i++) {
        if(!visited[i]) f(f, i);
        if(loop) {
            if(path.size() > 1) loops.emplace_back(path);
            path.clear();
            loop = false;
        }
    }
    ...
    vl memo(n, -1);
    bool ok = false;
    auto dfs = [&](auto f, int v, int cnt) -> ll {
        for(auto st : loops) {
            if(st.count(v)) {
                return memo[v] = size(st);
            }
        }
    ...
    rep(i,0,n) dfs(dfs, i, 0);
    rep(i,0,n) cerr << memo[i] << " \n"[i==n-1]; 
    ll ans = accumulate(all(memo), 0);
    cout << ans << endl;
    return 0;
}